...
export default function App({ teacher, onLogout }: { teacher: Teacher; onLogout: () => void }) {
  ...
  function go(page: PageId) { const update = () => setActive(page); if ("startViewTransition" in document) { // @ts-expect-error experimental
      document.startViewTransition(() => startTransition(update)); return; } startTransition(update); }
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_0%,rgba(192,160,98,0.14),transparent_26%),radial-gradient(circle_at_85%_18%,rgba(255,255,255,0.05),transparent_18%),linear-gradient(180deg,#050505_0%,#090909_48%,#070707_100%)] text-[#EDEDED]">
      <div className="mx-auto grid min-h-screen max-w-[1600px] gap-6 px-4 py-4 lg:grid-cols-[280px_minmax(0,1fr)] lg:px-6">
        <aside className="rounded-[32px] border border-[#C0A062]/18 bg-[rgba(12,12,12,0.82)] p-4 shadow-[0_30px_90px_rgba(0,0,0,0.55)] backdrop-blur-xl">
          <div className="flex items-center gap-3 rounded-[28px] border border-[#C0A062]/18 bg-[linear-gradient(135deg,rgba(192,160,98,0.1),rgba(255,255,255,0.03))] px-4 py-4"><div className="icon"><img src="/logo.png" alt="DataVista logo" /></div><div><button onClick={() => { setJumping(true); setVibgyorIndex((i) => (i + 1) % 7); go("dashboard"); setTimeout(() => setJumping(false), 700); }} className={cn("mt-1 inline-block whitespace-nowrap font-['Playfair_Display'] text-xl font-semibold uppercase tracking-[0.28em] text-[#E7D19A] transition-all duration-300 hover:scale-[1.03] hover:text-[#F2DEAE]", jumping && "animate-bounce")} style={{ textShadow: "0 0 18px rgba(192,160,98,0.22)" }}>DATA VISTA</button></div></div>
        ...
        <main className="rounded-[32px] border border-[#C0A062]/14 bg-[rgba(12,12,12,0.68)] p-4 shadow-[0_30px_90px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:p-5 xl:p-8" style={{ viewTransitionName: "app-page" }}>
...
function Hero() { return <Card className="overflow-hidden rounded-[36px] border-[#C0A062]/14 bg-[linear-gradient(135deg,rgba(192,160,98,0.12),rgba(14,12,10,0.98)_46%,rgba(192,160,98,0.16))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.4)] xl:p-8"><p className="text-xs uppercase tracking-[0.28em] text-[#C8B07A]">Command Center</p><h3 className="mt-3 max-w-2xl text-4xl font-semibold leading-tight text-[#FFF7E8] xl:text-5xl">One place to see class performance, risk signals, and next actions.</h3><p className="mt-4 max-w-2xl text-base text-[#B9B09E]">DataVista 2.0 brings dashboard analytics, attendance control, predictions, and AI insights into one artifact.</p></Card>; }
